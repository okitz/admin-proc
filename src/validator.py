...
class GraphValidator:
    ...
    def validate(self) -> Tuple[bool, List[str], List[str]]:
        ...
        self._check_ids()
        if self.errors: return False, self.errors, self.warnings # IDエラーなら即終了
            ...
            for sink_id in sinks:
                ...
                if node_type == "Digital_Object":
                    ...
                    self.warnings.append(
                        f"Dangling Data Warning: Digital Object '{node['label']}' is created but never submitted."
                    )
                ...
                # --- ケース2: 物理的成果物 (今回追加する部分) ---
                    ...
                    user_action_types = [
                        "Physical_Fill", "Physical_Copy_Store", "Physical_Print_Store",
                        "Physical_Acquire_Local", "External_Acquire", "Physical_Enclose"
                    ]
                ...
                # --- ケース4: その他 (Raw_Materialなど) ---
                    ...
                    self.warnings.append(
                        f"Dead-end Node Warning: '{node['label']}' [{node_type}] should not be a sink."
                    )
        ...
        for e in self.edges:
            ...
            if edge_type in RULES:
                ...
                if isinstance(valid_src, list):
                    ...
                    self.errors.append(f"Type Error: Edge '{edge_type}' cannot start from '{src_type}' ({e['source_id']}). Expected {valid_src}.")
                
                # Targetチェック
                if tgt_type != valid_tgt:
                    self.errors.append(f"Type Error: Edge '{edge_type}' cannot end at '{tgt_type}' ({e['target_id']}). Expected {valid_tgt}.")
    ...
    def _check_domain_logic(self):
        ...
        if len(submit_edges) > 1:
            self.warnings.append(f"Logic Warning: Multiple Digital_Submit edges found ({len(submit_edges)}). Data might not be aggregated.")
...
if __name__ == "__main__":
    ...
    data = {
        "nodes": [
            {"id": "d4", "type": "System", "label": "画面"},
            {"id": "d9", "type": "State", "label": "完了"}
        ],
        "edges": [
